"""
Smart Retriever - Query Routing and Intelligent Search

Features:
- Automatic query intent detection
- Route to appropriate document types
- Cross-lingual retrieval (Macedonian ↔ English)
...
class QueryIntent(Enum):
    ...
    TECHNICAL = "technical"          # DSA concepts, algorithms, code
    ADMINISTRATIVE = "administrative" # Course policies, grading, deadlines
    FAQ = "faq"                      # Common student questions
    MIXED = "mixed"                  # Combination


class SmartRetriever:
    """Intelligent retrieval with query routing"""
    ...
    def __init__(self, vector_store_manager):
        ...
        self.intent_patterns = {
            QueryIntent.FAQ: [
                r'\b(дали|можам|можеме|треба|потребно)\b',  # Question words
    ...
    def detect_intent(self, query: str) -> Tuple[QueryIntent, float]:
        ...
        if max_score == 0:
            return QueryIntent.TECHNICAL, 0.5  # Default to technical with low confidence
    ...
    def route_query(self, query: str, n_results: int = 5) -> Dict:
        ...
        if intent == QueryIntent.FAQ:
            results = self.vsm.search_faq_only(query, n_results=min(n_results, 5))
            strategy = "FAQ-only search"
            
        elif intent == QueryIntent.ADMINISTRATIVE:
            # Search FAQ + admin docs
            filter_metadata = {"$or": [
                {"is_faq": "True"},
                {"is_admin": "True"}
            ]}
            results = self.vsm.search(query, n_results, filter_metadata)
            strategy = "FAQ + Admin search"
            
        elif intent == QueryIntent.TECHNICAL:
            # Search lecture content (excluding FAQ/admin)
            doc_types = ["lecture_slides", "supplementary_slides", "textbook"]
            results = self.vsm.search_by_type(query, doc_types, n_results)
            strategy = "Technical content search"
            
        else:  # MIXED
            # Search all documents
            results = self.vsm.search(query, n_results)
    ...
    def search_technical(self, 
        ...
        Args:
            ...
            require_code: If True, only return chunks with code
            n_results: Number of results
        """
        if require_code:
            return self.vsm.search_with_code(query, n_results)
        else:
            doc_types = ["lecture_slides", "supplementary_slides", "textbook"]
            return self.vsm.search_by_type(query, doc_types, n_results)
    
    def search_faq(self, query: str, n_results: int = 3) -> Dict:
        """Search FAQ documents"""
        return self.vsm.search_faq_only(query, n_results)
    
    def search_admin(self, query: str, n_results: int = 3) -> Dict:
        """Search administrative documents"""
        return self.vsm.search_admin_only(query, n_results)
    ...
    def hybrid_search(self, 
        ...
        for result in semantic_results["results"]:
            ...
            if intent == QueryIntent.FAQ and metadata.get("is_faq") == "True":
                metadata_score += 0.3
            elif intent == QueryIntent.ADMINISTRATIVE and metadata.get("is_admin") == "True":
                metadata_score += 0.3
            elif intent == QueryIntent.TECHNICAL and metadata.get("doc_type") in ["lecture_slides", "supplementary_slides"]:
                metadata_score += 0.2
...
if __name__ == "__main__":
    ...
    print("Usage: SmartRetriever(vsm).route_query(query)")
